  ...
  actions: {
    ...
    async afterLoginAction(goHome?: boolean, data?: any): Promise<any | null> {
      ...
      if (sessionTimeout) {
        ...
        let redirect = '';
        if (router.currentRoute.value?.query?.redirect && router.currentRoute.value?.query?.redirect.includes('token=')) {
          const redirectUrl = new URL(router.currentRoute.value?.query?.redirect, 'http://localhost:3100');
          ...
          if (Object.keys(data).length === 0 || data == null || data == undefined) {
            ...
            this.setTenant(data.userInfo.loginTenantId);
          }
        } else {
