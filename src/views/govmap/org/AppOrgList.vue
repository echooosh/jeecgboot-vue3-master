<template>
  <div>
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection"
      ...
      <template #tableTitle>
        ...
        <a-button type="primary" preIcon="ant-design:export-outlined"
                  @click="onExportOfflinePacket"> 导出离线包
        </a-button>
    ...
    <!--字典弹窗-->
    <AppOrgModal @register="registerModal" @success="handleSuccess"/>
  </div>
</template>

<script lang="ts" name="govmap-appOrg" setup>
//ts语法
import {ref, computed, unref, toRaw, nextTick} from 'vue';
import {BasicTable, TableAction} from '/@/components/Table';
import {useModal} from '/@/components/Modal';
import {useListPage} from '/@/hooks/system/useListPage'
import AppOrgModal from './components/AppOrgModal.vue';
import {columns, searchFormSchema} from './AppOrg.data';
...
import {moveHall} from "/@/views/govmap/hall/AppHall.api";
...
const {prefixCls, tableContext, onExportXls, onImportXls} = useListPage({
  tableProps: {
    ...
    useSearchForm: false,
    beforeFetch(params) {
        Object.assign(params,{ column: 'sortNum', order: 'desc' } );
    },
    actionColumn: {
      width: 300,
      fixed: 'right'
...
async function handleDelete(record) {
  await deleteAppOrg({id: record.id}, importSuccess);
}
...
async function batchHandleDelete() {
  ...
  await batchDeleteAppOrg({id: ids}, importSuccess);
}
...
async function onMove(direction, record) {
  await moveOrg({id: record.id, direction: direction}, reload);
}
</script>
